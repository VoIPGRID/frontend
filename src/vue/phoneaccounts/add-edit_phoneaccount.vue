<Tabs :tabs=tabs class="content-page">
    ...
    <Tab slot="tabs" :data="tabs[0]">
        ...
        <Field name="country" type="select"
            ...
            :model.sync="phoneaccount.country"
            :placeholder="$t('Select a country...')"/>

        <Field name="n112_region" type="select"
            :label="$t('112 region')"
            :model.sync="phoneaccount.n112_region"
    ...
    <template slot="controls">
        <p class="control">
            <button class="button is-primary" :disabled="$v.$invalid" @click="upsertPhoneaccount(phoneaccount)">
                {{$t('Save changes')}}
            </button>
        </p>
        <p class="control">
